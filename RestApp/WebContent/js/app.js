...
App.controller("TodoCtrl", function ($scope, $http) {
	...
	var url = "http://ac00100.sytes.net:8080/RestApp/api/tasks";
	...
	$scope.addTodo = function () {
		...
		$http({
		    method: "POST",
		    url: "http://ac00100.sytes.net:8080/RestApp/api/addTask",
	...
	$scope.updateTask = function (item) {
		...
		$http({
		    method: "PUT",
		    url: "http://ac00100.sytes.net:8080/RestApp/api/updateTask",
		    data: {
		    	id : item.id,
	...
	$scope.deleteTodo = function (item) {
		$http({
		    method: "DELETE",
		    url: "http://ac00100.sytes.net:8080/RestApp/api/deleteTask/" + item.id,
		    headers: {"Content-Type": "application/json"}
		}).error(function (data, status, headers, config) {
			$scope.init();
        });
